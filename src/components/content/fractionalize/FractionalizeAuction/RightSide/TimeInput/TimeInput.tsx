...
function TimeInput({
	...
	const handleChangeInput = (e: InputChange) => {
		...
		if (
			...
			id === 'hour' &&
			range === 'to' &&
			Number(value) < Number(date['from'].hour)
		) {
			setAlert({
				checkNumber: false,
				bol: true,
				message: 'Try increasing the value',
			});
		}
		//check digit input
		value = parseNumber(value, setAlert, {
			checkNumber: true,
			bol: true,
		...
		if (
			!setInputValue(name, value, 'hour', range === 'from' ? 23 : 24, setAlert)
		)
			return;
		...
		if (range === 'to' && date['to'].hour === '24') {
			setAlert({
				checkNumber: true,
				bol: true,
				message: 'Try changing the values',
			});
			return;
		}
