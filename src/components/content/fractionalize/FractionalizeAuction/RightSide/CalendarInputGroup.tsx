...
function CalendarInputGroup() {
    const [alert, setAlert] = useState<AlertType>({bol: false, message: "", checkNumber: false})
    const initState = {from: {hour: "", minute: ""}, to: {hour: "", minute: ""}}
    const [date, setDate] = useState<initStateType>(initState)

    return (
        <div className={style.input}>
            <div className={style.input_group}>
                <TimeInput setAlert={setAlert}
                           date={date}
                           setDate={setDate}
                           range={"from"}/>
                <small>to</small>
            ...
            {
                alert.bol ? <small className={style.input_alert}>
